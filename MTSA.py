...
def fore_chow( stock1 , stock2 , stock1_trade , stock2_trade ):
    ...
    try:
        ...
        tau_h = np.dot(np.dot( e_t.T , np.linalg.inv(sigma_t) ) , e_t )
        ...
        if tau_h > float(chi2.ppf(0.90,k)):
...
#spread = 0.5 * np.log(stock1) - 0.5 * np.log(stock2)
